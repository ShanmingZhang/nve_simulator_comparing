 ...
 * return each virtual network embedding request timing list
 */
vector<int> RandomGenerator::getVirtualNetworkEmebeddingRequestsWithUniformDistribution() {
	return UniformDistribution();
}
/*
...
vector<int> RandomGenerator::ExponentialDistribution() {
	...
	vector<int> numbers;
	for (int index = 0; index < (CommonConstants::VNE_ELAPSED_TIME_UNITS); //3600
 ...
 * generate random numbers they are whether repeat according to amount and max - min
 */
vector<int> RandomGenerator::UniformDistribution() {
	unsigned SEED = std::chrono::system_clock::now().time_since_epoch().count();
	default_random_engine generator(SEED);
	uniform_int_distribution<int> distribution(0,
			CommonConstants::VNW_REQUEST_TIME_INTERVAL_UNITS);   // y
