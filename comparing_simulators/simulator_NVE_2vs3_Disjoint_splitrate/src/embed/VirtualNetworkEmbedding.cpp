...
BasePath* VirtualNetworkEmbedding::getOneDisjointPathWithComplexConstraintsOnSubstrateNetwork(
	...
	if (paths.size() != 0) {
		int index = 0;
		int middleIndex = paths.size() > 2 ? paths.size() -2 : paths.size() / 2;
...
void VirtualNetworkEmbedding::updateNveDataSwitchingEnergy(const int timing) {
	...
	for (multimap<int, EndUserRequest*>::iterator reqIter =
		...
		if (timing <= reqIter->second->getReqEndTiming()
			...
			dataVolume = dataVolume + (reqIter->second->getReqDataRate() * 1);

			if (reqIter->second->getComFlag() == 1) {

				comDataVolume = comDataVolume
						+ (reqIter->second->getReqDataRate() * 1);
			}
...
void VirtualNetworkEmbedding::updateCurrentRevenueVirLinkAndNodeRecords(
	...
	double totalSubCostBandwidth = 0.0;

	int sameVirNwLinks = 0;
	int sameVirNwNodes = 0;
	int sameVirNwEmbeddedBandwidth = 0.0;
	int sameVirNwSubCostBandwidth = 0.0;
	...
	for (map<int, VirNwGraph*>::iterator vnwIter = this->vNws.begin();
		...
		for (map<string, VirNwLink*>::iterator vlIter =
			...
			if (vnwIter->second->getSameFlag() == 1) {
				sameVirNwLinks = sameVirNwLinks + 1;
				sameVirNwEmbeddedBandwidth = sameVirNwEmbeddedBandwidth
						+ +vlIter->second->getVirLinkBandwidth();
			}

			// add for comparing cost
			for (vector<BasePath*>::iterator bpIter =
					vlIter->second->getVirLinkSubPaths().begin();
					bpIter != vlIter->second->getVirLinkSubPaths().end();
					++bpIter) {
				totalSubCostBandwidth = totalSubCostBandwidth
						+ (*bpIter)->getPathHops()
								* (*bpIter)->getPathBandwidth();

				if (vnwIter->second->getSameFlag() == 1) {
					sameVirNwSubCostBandwidth = sameVirNwSubCostBandwidth
							+ (*bpIter)->getPathHops()
									* (*bpIter)->getPathBandwidth();
				}
			}
		...
		if (vnwIter->second->getSameFlag() == 1) {
			sameVirNwNodes = sameVirNwNodes
					+ vnwIter->second->getVirNodes().size();
		}
	}
	...
	this->currentSameVirNwsRevenueVirLinkRecords.insert(
			make_pair(timing,
					make_pair(sameVirNwLinks / 2,
							sameVirNwEmbeddedBandwidth / 2)));
	...
	map<int, int>::iterator sIter =
			this->currentSameVirNwsRevenueVirNodeRecords.begin();
	this->currentSameVirNwsRevenueVirNodeRecords.insert(sIter,
			pair<int, int>(timing, sameVirNwNodes));

	// add for comparing cost
	map<int, double>::iterator cIter = this->currentCostSubLinkRecords.begin();
	this->currentCostSubLinkRecords.insert(cIter,
			pair<int, double>(timing, totalSubCostBandwidth / 2));

	// add for comparing cost
	map<int, double>::iterator scIter =
			this->currentSameVirNwsCostSubLinkRecords.begin();
	this->currentSameVirNwsCostSubLinkRecords.insert(scIter,
			pair<int, double>(timing, sameVirNwSubCostBandwidth / 2));
...
void VirtualNetworkEmbedding::updateCurrentSubLinkEmbeddingResourceInfo(
	...
	for (map<string, SubNwLink*>::iterator slIter =
		...
		if (slIter->second->getSubLinkEmbeddableBandwidth() >= minBw && slIter->second->getSubLinkEmbeddableBandwidth() <= maxBw) {
			amountAvaSubLinks++;
		}
	}
	...
	this->currentEmbeddableSubLinksInfo.insert(
				make_pair(timing, amountAvaSubLinks));
}
...
const map<int, int>& VirtualNetworkEmbedding::getCurrentSameVirNws() const {
	return currentSameVirNws;
}

void VirtualNetworkEmbedding::setCurrentSameVirNws(
		const map<int, int>& currentSameVirNws) {
	this->currentSameVirNws = currentSameVirNws;
}

void VirtualNetworkEmbedding::updateCurrentSameVirNws(const int timing) {

	int currentSameVirNws = 0;

	for (map<int, VirNwGraph*>::iterator vnIter = this->getNws().begin();
			vnIter != this->getNws().end(); ++vnIter) {
		if (vnIter->second->getSameFlag() == 1) {
			currentSameVirNws++;
		}
	}

	map<int, int>::iterator iter = this->currentSameVirNws.begin();
	this->currentSameVirNws.insert(iter,
			pair<int, int>(timing, currentSameVirNws));

}
const map<int, int>& VirtualNetworkEmbedding::getCurrentComVirNws() const {
	return currentComVirNws;
}

void VirtualNetworkEmbedding::setCurrentComVirNws(
		const map<int, int>& currentComVirNws) {
	this->currentComVirNws = currentComVirNws;
}

void VirtualNetworkEmbedding::updateCurrentComVirNws(const int timing) {
	map<int, int>::iterator iter = this->currentComVirNws.begin();

	this->currentComVirNws.insert(iter,
			pair<int, int>(timing, (int) this->getComVirNwNos().size()));
}

map<string, EndUserRequest*> VirtualNetworkEmbedding::getAvaliableEndUserRequests(
		const int timing, map<string, EndUserRequest*> euRequests) {

	map<string, EndUserRequest*> ava_euRequests;
	vector<string> reqNos;

	pair<multimap<int, string>::const_iterator,
			multimap<int, string>::const_iterator> conEndReqIter;
	conEndReqIter = this->getCongestionRecords().equal_range(timing);

	// embedding virtual link
	for (multimap<int, string>::const_iterator cIter = conEndReqIter.first;
			cIter != conEndReqIter.second; ++cIter) {
		vector<string>::iterator iter = reqNos.begin();
		reqNos.insert(iter, cIter->second);
	}

	for (map<string, EndUserRequest*>::iterator aIter = euRequests.begin();
			aIter != euRequests.end(); ++aIter) {
		if (find(reqNos.begin(), reqNos.end(), aIter->second->getReqId())
				== reqNos.end()) {
			ava_euRequests.insert(
					make_pair(aIter->second->getReqId(), aIter->second));
		}
	}

	return ava_euRequests;
}

const map<int, int>& VirtualNetworkEmbedding::getCurrentEmbeddableSubLinksInfo() const {
	return currentEmbeddableSubLinksInfo;
}

void VirtualNetworkEmbedding::setCurrentEmbeddableSubLinksInfo(
		const map<int, int>& currentEmbeddableSubLinksInfo) {
	this->currentEmbeddableSubLinksInfo = currentEmbeddableSubLinksInfo;
}


const map<int, double>& VirtualNetworkEmbedding::getCurrentCostSubLinkRecords() const {
	return currentCostSubLinkRecords;
}

void VirtualNetworkEmbedding::setCurrentCostSubLinkRecords(
		const map<int, double>& currentCostSubLinkRecords) {
	this->currentCostSubLinkRecords = currentCostSubLinkRecords;
}

const map<int, int>& VirtualNetworkEmbedding::getFailureEmbeddingRecords() const {
	return failureEmbeddingRecords;
}

void VirtualNetworkEmbedding::setFailureEmbeddingRecords(
		const map<int, int>& failureEmbeddingRecords) {
	this->failureEmbeddingRecords = failureEmbeddingRecords;
}

void VirtualNetworkEmbedding::updateFailureEmbeddingRecords(const int timing,
		const int count) {
	map<int, int>::iterator iter = this->failureEmbeddingRecords.begin();

	this->failureEmbeddingRecords.insert(iter, pair<int, int>(timing, count));
}


//
const map<int, double>& VirtualNetworkEmbedding::getCurrentSameVirNwsCostSubLinkRecords() const {
	return currentSameVirNwsCostSubLinkRecords;
}

void VirtualNetworkEmbedding::setCurrentSameVirNwsCostSubLinkRecords(
		const map<int, double>& currentSameVirNwsCostSubLinkRecords) {
	this->currentSameVirNwsCostSubLinkRecords =
			currentSameVirNwsCostSubLinkRecords;
}

const map<int, pair<int, double> >& VirtualNetworkEmbedding::getCurrentSameVirNwsRevenueVirLinkRecords() const {
	return currentSameVirNwsRevenueVirLinkRecords;
}

void VirtualNetworkEmbedding::setCurrentSameVirNwsRevenueVirLinkRecords(
		const map<int, pair<int, double> >& currentSameVirNwsRevenueVirLinkRecords) {
	this->currentSameVirNwsRevenueVirLinkRecords =
			currentSameVirNwsRevenueVirLinkRecords;
}

const map<int, int>& VirtualNetworkEmbedding::getCurrentSameVirNwsRevenueVirNodeRecords() const {
	return currentSameVirNwsRevenueVirNodeRecords;
}

void VirtualNetworkEmbedding::setCurrentSameVirNwsRevenueVirNodeRecords(
		const map<int, int>& currentSameVirNwsRevenueVirNodeRecords) {
	this->currentSameVirNwsRevenueVirNodeRecords =
			currentSameVirNwsRevenueVirNodeRecords;
}

const map<int, int>& VirtualNetworkEmbedding::getVirNwRequestRecords() const {
	return virNwRequestRecords;
}

void VirtualNetworkEmbedding::setVirNwRequestRecords(
		const map<int, int>& virNwRequestRecords) {
	this->virNwRequestRecords = virNwRequestRecords;
}

void VirtualNetworkEmbedding::updateVirNwRequestRecords(const int timing,
		const int count) {
	map<int, int>::iterator iter = this->virNwRequestRecords.begin();

	this->virNwRequestRecords.insert(iter, pair<int, int>(timing, count));
}

void VirtualNetworkEmbedding::OutputNveInfoToJson(const string fileName,
		const string type, const string description,
		VirtualNetworkEmbedding * nve) {
	CommonFunctions::OutputRecordsIntoJsonFile(fileName, type, description,
			nve->getCongestionRecords(), nve->getAmountVnsRecords(),
			nve->getCurrentNumberOfVns(),
			nve->getCurrentRevenueVirLinkRecords(),
			nve->getCurrentRevenueVirNodeRecords(),
			nve->getDataSwitchingEnergy(), nve->getTransportedDataVolume(),
			nve->getComDataSwitchingEnergy(),
			nve->getComTransportedDataVolume(),
			nve->getCurrentAvailableEndUserRequests(),

			nve->getSameVirNwDataSwitchingEnergy(),
			nve->getSameVirNwTransportedDataVolume(),
			nve->getCurrentCommonEndUserReqs(),
			nve->getCurrentEndingEndUserReqs(), nve->getCurrentEndingVirNws(),
			nve->getCurrentSubLinkResourceInfo(),

			nve->getCurrentEmbeddableSubLinksInfo(),

			nve->getCurrentSameVirNws(), nve->getCurrentComVirNws(),
			nve->getCurrentCostSubLinkRecords(),

			nve->getFailureEmbeddingRecords(),

			nve->getVirNwRequestRecords(),
			nve->getCurrentSameVirNwsRevenueVirLinkRecords(),
			nve->getCurrentSameVirNwsRevenueVirNodeRecords(),
			nve->getCurrentSameVirNwsCostSubLinkRecords());
}

void VirtualNetworkEmbedding::OutPutNveInfoToImageFile(const string type_1,
		const string type_2, VirtualNetworkEmbedding * nve_1,
		VirtualNetworkEmbedding * nve_2) {
	CommonFunctions::OutputRecordsIntoImageFile(type_1, type_2,
			nve_1->getCongestionRecords(), nve_2->getCongestionRecords(),
			nve_1->getAmountVnsRecords(), nve_2->getAmountVnsRecords(),
			nve_1->getCurrentNumberOfVns(), nve_2->getCurrentNumberOfVns(),
			...
			nve_1->getDataSwitchingEnergy(), nve_2->getDataSwitchingEnergy(),
			...
			nve_1->getCurrentEndingVirNws(), nve_2->getCurrentEndingVirNws(),
			...
			nve_1->getCurrentSameVirNws(), nve_2->getCurrentSameVirNws(),

			nve_1->getCurrentComVirNws(), nve_2->getCurrentComVirNws(),

			nve_1->getCurrentCostSubLinkRecords(),
			nve_2->getCurrentCostSubLinkRecords(),

			nve_1->getFailureEmbeddingRecords(),
			nve_2->getFailureEmbeddingRecords(),

			//
			nve_1->getVirNwRequestRecords(), nve_2->getVirNwRequestRecords(),

			nve_1->getCurrentSameVirNwsRevenueVirLinkRecords(),
			nve_2->getCurrentSameVirNwsRevenueVirLinkRecords(),

			nve_1->getCurrentSameVirNwsRevenueVirNodeRecords(),
			nve_2->getCurrentSameVirNwsRevenueVirNodeRecords(),

			nve_1->getCurrentSameVirNwsCostSubLinkRecords(),
			nve_2->getCurrentSameVirNwsCostSubLinkRecords());
}
