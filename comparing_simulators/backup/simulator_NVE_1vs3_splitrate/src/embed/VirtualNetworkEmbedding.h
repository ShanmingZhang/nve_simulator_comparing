...
public:
	...
	void updateCurrentSubLinkEmbeddingResourceInfo(const int timing,
			const double minBw, const double maxBw);
	...
	void updateSameVirNwDataSwitchingEnergy(const int timing,
			const double sameVirDataSwitchingEnergy);
	...
	void updateSameVirNwTransportedDataVolume(const int timing,
			const double sameVirTransportedDataVolume);

	const map<int, int>& getCurrentSameVirNws() const;
	void setCurrentSameVirNws(const map<int, int>& currentSameVirNws);
	void updateCurrentSameVirNws(const int timing);

	const map<int, int>& getCurrentComVirNws() const;
	void setCurrentComVirNws(const map<int, int>& currentComVirNws);
	void updateCurrentComVirNws(const int timing);
	/*
	 * get available end-user request from total end-user requests and congestion end-user requests
	 */
	map<string, EndUserRequest*> getAvaliableEndUserRequests(const int timing,
			map<string, EndUserRequest*> euRequests);
	const map<int, int>& getCurrentEmbeddableSubLinksInfo() const;
	void setCurrentEmbeddableSubLinksInfo(
			const map<int, int>& currentEmbeddableSubLinksInfo);
	const map<int, double>& getCurrentCostSubLinkRecords() const;
	void setCurrentCostSubLinkRecords(
			const map<int, double>& currentCostSubLinkRecords);
	const map<int, int>& getFailureEmbeddingRecords() const;
	void setFailureEmbeddingRecords(
			const map<int, int>& failureEmbeddingRecords);

	void updateFailureEmbeddingRecords(const int timing, const int count);
	const map<int, double>& getCurrentSameVirNwsCostSubLinkRecords() const;
	void setCurrentSameVirNwsCostSubLinkRecords(
			const map<int, double>& currentSameVirNwsCostSubLinkRecords);
	const map<int, pair<int, double> >& getCurrentSameVirNwsRevenueVirLinkRecords() const;
	void setCurrentSameVirNwsRevenueVirLinkRecords(
			const map<int, pair<int, double> >& currentSameVirNwsRevenueVirLinkRecords);
	const map<int, int>& getCurrentSameVirNwsRevenueVirNodeRecords() const;
	void setCurrentSameVirNwsRevenueVirNodeRecords(
			const map<int, int>& currentSameVirNwsRevenueVirNodeRecords);


	const map<int, int>& getVirNwRequestRecords() const;
	void setVirNwRequestRecords(const map<int, int>& virNwRequestRecords);

	void updateVirNwRequestRecords(const int timing, const int count);


	static void OutputNveInfoToJson(const string fileName, const string type,
			const string description, VirtualNetworkEmbedding * nve);
	static void OutPutNveInfoToImageFile(const string type_1,
...
private:
	...
	// int: timing,  int: virtual network request;
	...
	//int: timing, double: int: amount of virtual links, double: current total cost(bandwidth) of substrate links
	...
	/*
	 * int: timing,
	 * int: current amount of substrate links(embed Bw >= 30 M && embed Bw <= 100 M)
	...
	map<int, int> currentSameVirNws;
	map<int, int> currentComVirNws;
