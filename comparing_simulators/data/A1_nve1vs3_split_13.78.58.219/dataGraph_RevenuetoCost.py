...
def showRevenueToCostRatioImage( key_1, key_2, limits, xlimits, record_1, record_2, record_3, record_4, record_5, ytxt, xtxt, title):
	timing_1 = []
	timing_2 = []
	timing_3 = []
	timing_4 = []
	...
	for item_1, item_2 in zip(record_1[key_1], record_1[key_2]):
		...
		if (index > 1 ) and ((index % limits) == 0):
			...
			if sum_1 == 0 or sum_2 == 0:
				...
				values_1.append((float(sum_1)/float(sum_2)))
			sum_1 = 0
			sum_2 = 0
	...
	for item_1, item_2 in zip(record_2[key_1], record_2[key_2]):
		...
		if (index > 1 ) and ((index % limits) == 0):
			...
			if sum_1 == 0 or sum_2 == 0:
				...
				values_2.append((float(sum_1)/float(sum_2)))
			sum_1 = 0
			sum_2 = 0
	...
	for item_1, item_2 in zip(record_3[key_1], record_3[key_2]):
		...
		if (index > 1 ) and ((index % limits) == 0):
			...
			if sum_1 == 0 or sum_2 == 0:
				...
				values_3.append((float(sum_1)/float(sum_2)))
			sum_1 = 0
			sum_2 = 0
	...
	for item_1, item_2 in zip(record_4[key_1], record_4[key_2]):
		...
		if (index > 1 ) and ((index % limits) == 0):
			...
			if sum_1 == 0 or sum_2 == 0:
				...
				values_4.append((float(sum_1)/float(sum_2)))
			sum_1 = 0
			sum_2 = 0
	...
	for item_1, item_2 in zip(record_5[key_1], record_5[key_2]):
		...
		if (index > 1 ) and ((index % limits) == 0):
			...
			if sum_1 == 0 or sum_2 == 0:
				...
				values_5.append((float(sum_1)/float(sum_2)))
			sum_1 = 0
			sum_2 = 0
	...
	plt.ylim(0, y_limits * 1.13 )
	
	plt.grid(True)
	#plt.title('(E) Virtual Network Revenue To Substrate Cost Ratio', fontsize=17)
...
#key_11 = 'SublinkCost'
#Revenue to subcost = Revenue / SubCost
showRevenueToCostRatioImage( 'EmbeddedBandwidth', 'SublinkCost', limits, xlimits, recordJson_1, recordJson_2, recordJson_3, recordJson_4, recordJson_5,'Revenue To  Cost Ratio', 'Time Unit', 'A3 Evaluation of Virtual Network Revenue To Substrate Cost')
