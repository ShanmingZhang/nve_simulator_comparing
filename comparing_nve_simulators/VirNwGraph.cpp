...
int VirNwGraph::compareTwoVirtualNetworks(VirNwGraph* virNw_1,
		VirNwGraph* virNw_2) {
	MessageSet::printInfoMessage(
			" VirNwGraph::compareTwoVirtualNetworks Start");
	...
	for (map<string, VirNwLink*>::iterator vlIter_1 =
		...
		VirNwLink* tmp_virNwLink = virNw_2->getVirLink(
				vlIter_1->second->getVirLinkId());

		if ( NULL != tmp_virNwLink) {

			cout << "  compare step  8 " << endl;
			if (vlIter_1->second->getVirLinkAvailableBandwidth()
					!= tmp_virNwLink->getVirLinkAvailableBandwidth()) {
				return 0;
			}

			cout << "  compare step  9 " << endl;
			if (vlIter_1->second->getVirLinkBandwidth()
					!= tmp_virNwLink->getVirLinkBandwidth()) {
				return 0;
			}

			cout << "  compare step  10 " << endl;
			if (vlIter_1->second->getVirLinkFrom()
					!= tmp_virNwLink->getVirLinkFrom()) {
				return 0;
			}

			cout << "  compare step  11 " << endl;
			if (vlIter_1->second->getVirLinkSubPaths().size()
					!= tmp_virNwLink->getVirLinkSubPaths().size()) {
				return 0;
			}

			cout << "  compare step  12 " << endl;
			if (vlIter_1->second->getVirLinkSubPaths().size() == 1
					&& tmp_virNwLink->getVirLinkSubPaths().size() == 1) {

				if (vlIter_1->second->getVirLinkSubPaths()[0]->getPathInfo().size()
						!= tmp_virNwLink->getVirLinkSubPaths()[0]->getPathInfo().size()) {
					return 0;
				}

				//complatly same virtual network
				cout << "  compare step  12_1 " << endl;
				if (vlIter_1->second->getVirLinkSubPaths()[0]->getPathInfo()
						!= tmp_virNwLink->getVirLinkSubPaths()[0]->getPathInfo()) {
					return 0;
				}
			}

			cout << " compare step 13 " << endl;
			if (vlIter_1->second->getVirLinkSubPaths().size() == 2
					&& tmp_virNwLink->getVirLinkSubPaths().size() == 2) {

				unsigned int a =
						vlIter_1->second->getVirLinkSubPaths()[0]->getPathInfo().size();

				unsigned int b =
						tmp_virNwLink->getVirLinkSubPaths()[0]->getPathInfo().size();

				unsigned int c =
						vlIter_1->second->getVirLinkSubPaths()[1]->getPathInfo().size();

				unsigned int d =
						tmp_virNwLink->getVirLinkSubPaths()[1]->getPathInfo().size();
				...
				if (a == b) {
					...
					cout << " compare step 13_1 " << endl;
					if (vlIter_1->second->getVirLinkSubPaths()[0]->getPathInfo()
							!= tmp_virNwLink->getVirLinkSubPaths()[0]->getPathInfo()) {
						return 0;
					}

					cout << " compare step 13_2 " << endl;
					if (vlIter_1->second->getVirLinkSubPaths()[1]->getPathInfo()
							!= tmp_virNwLink->getVirLinkSubPaths()[1]->getPathInfo()) {
						return 0;
					}
					...
					cout << " compare step 13_3 " << endl;
					if (vlIter_1->second->getVirLinkSubPaths()[0]->getPathInfo()
							!= tmp_virNwLink->getVirLinkSubPaths()[1]->getPathInfo()) {
						return 0;
					}

					cout << " compare step 13_4 " << endl;
					if (vlIter_1->second->getVirLinkSubPaths()[1]->getPathInfo()
							!= tmp_virNwLink->getVirLinkSubPaths()[0]->getPathInfo()) {
						return 0;
					}
	...
	return 1;
	MessageSet::printInfoMessage(" VirNwGraph::compareTwoVirtualNetworks End");
}
